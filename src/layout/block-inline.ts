...
import { parseStyleAttribute, pxLength, resolveLength, makeStyle, type BorderRadius, type ComputedStyle, type Color, type Declaration, type DecorationLine, type DisplayValue, type ListStyleType, type PseudoBox, type VerticalAlign, type Viewport } from './css.js';
...
export function resolveStyles(
  ...
  const walk = (el: P5Element, d: StyleDefaults): void => {
    ...
    const style = makeStyle(decls, {
      ...
      whiteSpaceDefault: d.whiteSpace,
    });
...
function walkLine(pieces: InlinePiece[], style: ComputedStyle): WalkedLine {
  ...
  const flush = (): void => {
    if (!runText) return;
    const w = measureTextWidth(runText, runStyle!.fontSize, runStyle!.family, runStyle!.letterSpacing, runStyle!.fontWeight, runStyle!.fontStyle);
    runs.push({ text: runText, x: runX, width: w, style: runStyle!, owner: runOwner });
    runText = '';
    runStyle = null;
    runOwner = null;
  };
  const spaceW = (s: TextRunStyle): number => measureTextWidth(' ', s.fontSize, s.family, s.letterSpacing, s.fontWeight, s.fontStyle);
  for (const p of pieces) {
    if (p.kind === 'space') {
      prevWasSpace = true;
      continue;
