...
| ID | Sev | Area | Finding | Evidence (gen-4 HEAD 9523c2c) | Fix direction | Status | Attempts |
| --- | --- | --- | --- | --- | --- | --- | --- |
...
| QA-11 | medium | config | `safari.ts:34` hardcodes `/home/sagi/.local/share/fonts/HackNerdFont-Regular.ttf`, violating the stated no-home-dir policy (`chrome.ts` `fontPath` pattern; vendored copy exists at `fonts/HackNerdFont-Regular.ttf`); `chrome.ts:131` lists `'Droid Sans Japanese'` in scriptCoverage with no registration/fallback reference (verify, then remove); safari config is wired but has no verification loop (unlike firefox) — probe-only per charter §4 until WebKit oracle lands | gen-4 re-confirmed: `src/config/safari.ts:34` hardcoded home-dir path persists (vs `chrome.ts:62` `fontPath(env, repoFile)` and vendored `fonts/HackNerdFont-Regular.ttf`); `src/config/chrome.ts:131` `'Droid Sans Japanese'` row still present with no registration/fallback reference (`chrome.ts:83` registers Droid Sans Fallback, not Japanese) | route through `fontPath(env, repoFile)`; drop dead row after grep-disproof of references; ledger marks safari probe-only | open (gen-5 re-confirmed: `src/config/safari.ts:34` hardcoded home-dir path persists (vs `chrome.ts:40` `fontPath(env, repoFile)` and vendored `fonts/HackNerdFont-Regular.ttf`); `src/config/chrome.ts:131` `'Droid Sans Japanese': ['Hani']` row still present with no registration/fallback reference (`chrome.ts:83` registers Droid Sans Fallback, scriptFallback at :115 maps Hani → Droid Sans Fallback)). **FINAL at N=5**: the stale gen-0 `safari-config-fix` was never dispatched and no fix task was created this generation (medium severity, behind the high-severity QA-03/QA-04/QA-05 in the 2-task budget) — the finding remains open, terminal; the gen-0 spec still stands in tasks/ if the daemon ever dispatches it | 0 |
...
| QA-03 (+ QA-04 + QA-08) | `qa03-charter-deferred-enforcement` | 5 | (created, not yet landed) | Re-verify at HEAD, then (a) restructure charter §11 Deferred prose (charter.md:223+) into a parseable table (Absent surface | Status | Evidence) preserving every claim; (b) extend check-charter.mjs to assert token presence/absence per status, proven with a deliberate-contradiction probe; (c) regenerate coverage-matrix.md's deferred table from the new schema + correct the stale "all 53 rows" claim (106 §11 data rows at gen-5 HEAD); (d) refresh parity.md's census (4 typed gaps at HEAD) + Latest Run from real runs | created at the N=5 close — the chain's final fix-task budget (highest-severity verified open findings). Supersedes the stale gen-0 `charter-deferred-enforcement` spec (never dispatched across 5 generations). Outcome (fixed / partial / disproven) will be recorded by the task in this ledger when it lands. |
| QA-05 | `qa05-verify-gate-coverage` | 5 | (created, not yet landed) | Re-verify at HEAD, then (a) enumerate scripts/verify-*.mjs vs package.json; (b) wire every environment-independent verify script into verify:all (or a documented tier — never leave a script unwired); (c) add npm entries for verify:rtl + verify:paint-fallback; (d) fix the parity.md:24 overclaim (wording only, numbers from real runs) | created at the N=5 close — the chain's final fix-task budget (highest-severity verified open findings). Supersedes the stale gen-0 `verify-gate-coverage` spec (never dispatched across 5 generations). Outcome (fixed / partial / disproven) will be recorded by the task in this ledger when it lands. |
