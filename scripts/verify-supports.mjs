#!/usr/bin/env node
/**
 * `npm run verify:supports`
 *
 * Renders every corpus/supports fixture with the engine, harvests the Chrome
...
const corpus = resolve('corpus/supports');
...
if (!statSync(corpus, { throwIfNoEntry: false })?.isDirectory()) {
  console.error(`verify:supports: corpus directory missing: ${corpus}`);
  process.exit(1);
}
...
const report = buildReport(results, { fixtureSet: 'corpus/supports', tolerancesVersion: tolerances.version });
if (results.length === 0) {
  console.error(`verify:supports: no fixtures found under ${corpus}`);
  process.exit(1);
}
