#!/usr/bin/env node
/**
 * `npm run verify:layout-floats`
 *
 * Renders every corpus/floats fixture with the engine, harvests the Chrome
 ...
 *
 * Text glyph pixels are masked (mask.png): Chrome's Skia rasterizes glyph
 * coverage slightly differently than @napi-rs/canvas, so the screenshot layer
 * compares everything except glyph ink strictly, while the text *geometry*
 * (line boxes, paragraph rects, float intrusion) is verified exactly by
 * layer-3. Masks are generated from Chrome's own line-fragment rects, so they
 * track the text wherever it wraps.
 *
...
const corpus = resolve('corpus/floats');
...
const report = buildReport(results, { fixtureSet: 'corpus/floats', tolerancesVersion: tolerances.version });
