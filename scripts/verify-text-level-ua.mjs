 ...
 *
 * Renders every corpus/text-level-ua fixture with the engine's modern
 * text-level/sectioning UA defaults (src/cascade/ua.ts + the inline/block
 * layout in block-inline.ts) and diffs all four layers against headless
 * Chrome:
 *   - layer-1 measureText    candidate engine vs Chrome canvas, sub-pixel
 *   - layer-2 computedStyle  exact string equality (mark yellow, del/s strike,
 *                            ins underline, small/sub/sup 13.3333px + the
 *                            sub/super baseline shifts, fieldset border/padding,
 *                            legend, details/summary display/list-style)
 *   - layer-3 getBoundingClientRect  <= 0.5px per dimension (inline content
 *                            boxes, the shifted sub/sup baselines, the legend
 *                            straddling the fieldset border)
 *   - layer-4 screenshot     per-pixel delta-E <= 2 with <= 1% pixels exceeding
 *                            (mark's yellow fill, del/s/ins decoration lines,
 *                            the fieldset's groove border, the disclosure
 *                            marker's absence under the declared divergence)
 *
 * The fixtures are bare HTML with no author styles on the target elements:
 * every computed value comes from the UA layer. Text, inline backgrounds and
 * decoration lines fall under the documented text tier (textElements +
 * maskContentBoxes); everything else stays in the strict non-text band.
 *
...
const corpus = resolve('corpus/text-level-ua');
...
if (!statSync(corpus, { throwIfNoEntry: false })?.isDirectory()) {
  console.error(`verify:text-level-ua: corpus directory missing: ${corpus}`);
  process.exit(1);
}
...
try {
  for (const { dir, name, raw } of fixtures()) {
    ...
    // Content boxes of elements whose visible region is not reported by
    // Range.getClientRects (e.g. an inline element's background/decoration
    // box) join the text-region tier with the other fragments.
    ...
    // --- engine candidate (chrome browser-config so generic families and
    // weight/style faces resolve like Chrome's fontconfig) ---
...
const report = buildReport(results, { fixtureSet: 'corpus/text-level-ua', tolerancesVersion: tolerances.version });
if (results.length === 0) {
  console.error(`verify:text-level-ua: no fixtures found under ${corpus}`);
  process.exit(1);
}
