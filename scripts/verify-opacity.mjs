#!/usr/bin/env node
/**
 * `npm run verify:opacity`
 *
 * Renders every corpus/opacity fixture with the engine and diffs layer-by-layer
 * against the Chrome oracle:
 *   - layer-2 computedStyle  exact string equality for `opacity`
 *   - layer-3 getBoundingClientRect  <= 0.5px per dimension
 *   - layer-4 screenshot  per-pixel delta-E <= 2 with <= 1% exceeding
 *     (opacity fixtures are geometric — no text — so nothing falls under the
 *     documented text tier)
 *
 * The corpus exercises subtree compositing: faded subtrees (disabled-style),
 * fade-through-translucent-child, overlay dims, nested opacity, and the
 * opacity<1 stacking-context interaction with negative-z descendants. The
 * engine composites each opacity<1 subtree onto an offscreen surface and blends
 * it at the element alpha, matching Chrome's raster within the charter §4 band.
 *
...
const corpus = resolve('corpus/opacity');
...
if (!statSync(corpus, { throwIfNoEntry: false })?.isDirectory()) {
  console.error(`verify:opacity: corpus directory missing: ${corpus}`);
  process.exit(1);
}
...
try {
  for (const { dir, name, raw } of fixtures()) {
    ...
    writeFileSync(
      ...
      JSON.stringify({ measureText: {}, computedStyle: out.computedStyles, rect: out.rects }, null, 2) + '\n',
    );
    ...
    const fixture = {
      ...
      candidate: { measureText: {}, computedStyle: out.computedStyles, rect: out.rects },
      width,
      height,
    };

    results.push(evaluateFixture(fixture));
    console.log(`verified ${name}: ${width}x${height}, ${fragments.length} text fragments, ${mask.reduce((a, b) => a + b, 0)} masked`);
  }
} finally {
  await browser.close();
}

const report = buildReport(results, { fixtureSet: 'corpus/opacity', tolerancesVersion: tolerances.version });
if (results.length === 0) {
  console.error(`verify:opacity: no fixtures found under ${corpus}`);
  process.exit(1);
}
