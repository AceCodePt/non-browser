#!/usr/bin/env node
/**
 * `npm run verify:display-contents`
 *
 * Renders every corpus/display-contents fixture with the engine, harvests the Chrome
 * oracle quantities for the same HTML, and diffs layer-by-layer:
 *   - layer-1 measureText  (candidate engine vs Chrome canvas)
 *   - layer-2 computedStyle  exact string equality (display on the contents
 *     element itself, e.g. 'contents' in flow and 'none' on replaced elements)
 ...
 * masked (mask.png). The flex geometry itself (item sizing, grow/shrink,
 * wrapping, alignment) is verified pixel-exactly by layer-3 and by the
 * unmasked box pixels on layer-4.
 *
...
const corpus = resolve('corpus/display-contents');
...
try {
  for (const { dir, name, raw } of fixtures()) {
    ...
    writeFileSync(
      ...
      JSON.stringify({ measureText: candidateMeasure, computedStyle: out.computedStyles, rect: candidateRects }, null, 2) + '\n',
    );
...
const report = buildReport(results, { fixtureSet: 'corpus/display-contents', tolerancesVersion: tolerances.version });
