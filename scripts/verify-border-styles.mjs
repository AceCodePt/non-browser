#!/usr/bin/env node
/**
 * `npm run verify:border-styles`
 *
 * Renders every corpus/border-styles fixture with the engine, harvests the
 * Chrome oracle quantities for the same HTML, and diffs layer-by-layer:
 *   - layer-1 measureText  (candidate engine vs Chrome canvas)
 *   - layer-2 computedStyle  exact string equality (border style/width/color
 *     longhands — dashed/dotted/double/groove/ridge/hidden compute like Chrome)
 *   - layer-3 getBoundingClientRect  <= 0.5px per dimension (border-width
 *     contributes to the box identically for every style; hidden forces the
 *     used width to 0, matching none)
 ...
 *
 * The non-solid styles (dash/dot lengths scaled from border-width, double's two
 * lines with gap — solid fallback below 3px, groove/ridge edge shading) and
 * their interplay with border-radius are verified pixel-exactly against Chrome.
 *
...
const corpus = resolve('corpus/border-styles');
...
if (!statSync(corpus, { throwIfNoEntry: false })?.isDirectory()) {
  console.error(`verify:border-radius: corpus directory missing: ${corpus}`);
  process.exit(1);
}
...
const report = buildReport(results, { fixtureSet: 'corpus/border-styles', tolerancesVersion: tolerances.version });
if (results.length === 0) {
  console.error(`verify:border-radius: no fixtures found under ${corpus}`);
  process.exit(1);
}
