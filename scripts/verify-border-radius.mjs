#!/usr/bin/env node
/**
 * `npm run verify:border-radius`
 *
 * Renders every corpus/border-radius fixture with the engine, harvests the
 * Chrome oracle quantities for the same HTML, and diffs layer-by-layer:
 *   - layer-1 measureText  (candidate engine vs Chrome canvas)
 *   - layer-2 computedStyle  exact string equality (border-radius + longhands)
 *   - layer-3 getBoundingClientRect  <= 0.5px per dimension (geometry is
 *     unchanged by border-radius — the border-box rects are the same as for a
 *     plain box)
 ...
 *
 * The rounded shapes (uniform radius, 50% circle, elliptical rx/ry corners,
 * per-corner radii with overlap reduction) and the overflow:hidden rounded
 * clip on children are verified pixel-exactly against Chrome.
 *
...
const corpus = resolve('corpus/border-radius');
...
const report = buildReport(results, { fixtureSet: 'corpus/border-radius', tolerancesVersion: tolerances.version });
if (results.length === 0) {
