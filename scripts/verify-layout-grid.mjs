#!/usr/bin/env node
/**
 * `npm run verify:layout-grid`
 *
 * Renders every corpus/grid fixture with the engine, harvests the Chrome
 ...
 *
 * Text glyph pixels are masked (mask.png) exactly as in the floats corpus;
 * the grid geometry itself (track sizes, item placement, alignment) is
 * verified pixel-exactly by layer-3 and by the unmasked box pixels on layer-4.
 *
...
const corpus = resolve('corpus/grid');
...
const report = buildReport(results, { fixtureSet: 'corpus/grid', tolerancesVersion: tolerances.version });
