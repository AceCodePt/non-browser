#!/usr/bin/env node
/**
 * `npm run verify:custom-properties`
 *
 * Renders every corpus/custom-properties fixture with the engine, harvests the Chrome
...
const corpus = resolve('corpus/custom-properties');
...
if (!statSync(corpus, { throwIfNoEntry: false })?.isDirectory()) {
  console.error(`verify:custom-properties: corpus directory missing: ${corpus}`);
  process.exit(1);
}
...
const report = buildReport(results, { fixtureSet: 'corpus/custom-properties', tolerancesVersion: tolerances.version });
if (results.length === 0) {
  console.error(`verify:custom-properties: no fixtures found under ${corpus}`);
  process.exit(1);
}
