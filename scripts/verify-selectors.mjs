 ...
 *
 * Renders every corpus/selectors fixture with the engine, harvests the Chrome
...
const corpus = resolve('corpus/selectors');
...
if (!statSync(corpus, { throwIfNoEntry: false })?.isDirectory()) {
  console.error(`verify:selectors: corpus directory missing: ${corpus}`);
  process.exit(1);
}
...
const report = buildReport(results, { fixtureSet: 'corpus/selectors', tolerancesVersion: tolerances.version });
if (results.length === 0) {
  console.error(`verify:selectors: no fixtures found under ${corpus}`);
  process.exit(1);
}
