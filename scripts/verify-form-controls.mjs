 ...
 *
 * Renders every corpus/form-controls fixture with the engine's form-control
 * defaults (src/cascade/ua.ts + src/layout/controls.ts) and diffs all four
 * layers against headless Chrome:
 *   - layer-1 measureText    candidate engine vs Chrome canvas, sub-pixel
 *   - layer-2 computedStyle  exact string equality (display/border/padding/
 *                            background/font values of the UA defaults)
 *   - layer-3 getBoundingClientRect  <= 0.5px per dimension (control sizing:
 *                            size/cols attributes, theme metric heights)
 *   - layer-4 screenshot     per-pixel delta-E <= 2 with <= 1% pixels exceeding
 *                            (the theme-painted control art; label text under
 *                            the documented text tier)
 *
 * The fixtures are bare HTML with no author styles on the controls: every
 * computed value comes from the UA layer and every painted pixel from the
 * theme path, so this proves the engine's default form-control rendering
 * matches Chrome's appearance:auto controls.
 *
 ...
 * Exits 0 only when every fixture passes every layer.
 */
...
const corpus = resolve('corpus/form-controls');
...
if (!statSync(corpus, { throwIfNoEntry: false })?.isDirectory()) {
  console.error(`verify:form-controls: corpus directory missing: ${corpus}`);
  process.exit(1);
}
...
try {
  for (const { dir, name, raw } of fixtures()) {
    ...
    // Value-label content boxes (h.maskContentBoxes): an input button's label
    // is shadow text with no DOM text node, so Range.getClientRects reports
    // nothing. The content box is derived from Chrome's own rect and computed
    // border/padding — the region where Chrome paints the value text — and
    // joins the text-region tier with the other fragments.
    ...
    // --- engine candidate (chrome browser-config so the control font stack
    // Arial/monospace resolves to the same registered faces as Chrome) ---
...
const report = buildReport(results, { fixtureSet: 'corpus/form-controls', tolerancesVersion: tolerances.version });
if (results.length === 0) {
  console.error(`verify:form-controls: no fixtures found under ${corpus}`);
  process.exit(1);
}
