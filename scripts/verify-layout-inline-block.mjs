#!/usr/bin/env node
/**
 * `npm run verify:inline-block`
 *
 * Renders every corpus/inline-block fixture with the engine and diffs all four
 * layers against headless Chrome (Playwright), exactly as the charter §2
 * prescribes:
 ...
 *
 * These fixtures prove that display:inline-block boxes sit inline on the line
 * with surrounding text (rect x/y within 0.5px), size shrink-to-fit (or to
 * their authored width/height/margins, per box-sizing), align their baselines
 * with the surrounding text (css-inline-3), respect inline whitespace, and
 * wrap across lines. The inline-block's background/border pixels are compared
 * strictly (only the text spans listed in `textElements` are compared under
...
const corpus = resolve('corpus/inline-block');
 ...
 * either rasterizer paints as anything other than pure white. These are the
 * glyph pixels where rasterization policy can differ, so they are compared
...
if (!statSync(corpus, { throwIfNoEntry: false })?.isDirectory()) {
  console.error(`verify:inline-block: corpus directory missing: ${corpus}`);
  process.exit(1);
}
...
const report = buildReport(results, { fixtureSet: 'corpus/inline-block', tolerancesVersion: tolerances.version });
if (results.length === 0) {
  console.error(`verify:inline-block: no fixtures found under ${corpus}`);
  process.exit(1);
}
