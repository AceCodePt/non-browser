#!/usr/bin/env node
/**
 * `npm run verify:selectors-structural`
 *
 * Renders every corpus/selectors-structural fixture with the engine, harvests the Chrome
 * oracle quantities for the same HTML, and diffs layer-by-layer:
 *   - layer-2 computedStyle  exact string equality (every modern color form:
 *     hsl()/hsla() comma + space + hue units, space-separated rgb() with
 *     /alpha, the kept legacy comma rgb(), 4/8-digit hex, the full named set,
 *     currentColor, and the invalid-declaration drop semantics)
...
const corpus = resolve('corpus/selectors-structural');
...
if (!statSync(corpus, { throwIfNoEntry: false })?.isDirectory()) {
  console.error(`verify:selectors-structural: corpus directory missing: ${corpus}`);
  process.exit(1);
}
...
try {
  for (const { dir, name, raw } of fixtures()) {
    ...
    console.log(`verified ${name}: ${width}x${height}, ${fragments.length} text fragments`);
  }
} finally {
  await browser.close();
}

const report = buildReport(results, { fixtureSet: 'corpus/selectors-structural', tolerancesVersion: tolerances.version });
if (results.length === 0) {
  console.error(`verify:selectors-structural: no fixtures found under ${corpus}`);
  process.exit(1);
}
