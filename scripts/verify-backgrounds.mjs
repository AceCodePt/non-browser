 ...
 *
 * Renders every corpus/backgrounds fixture with the engine, harvests the
 * Chrome oracle quantities for the same HTML, and diffs layer-by-layer:
 *   - layer-1 measureText  (no background fixtures carry text)
 *   - layer-2 computedStyle  exact string equality (background longhands +
 *     the background shorthand serialization)
 ...
 *     (tolerances.json layers.screenshot.text)
 *
 * Covers gradient generation (linear/radial stops, angles, keywords, hard
 * stops, repeating), layered stacking, sizing/positioning/repeat, clip/origin
 * (with border-radius), the full background shorthand, and the chartered-out
 * url() raster background (parsed + serialized, never painted).
 *
...
const corpus = resolve('corpus/backgrounds');
...
if (!statSync(corpus, { throwIfNoEntry: false })?.isDirectory()) {
  console.error(`verify:backgrounds: corpus directory missing: ${corpus}`);
  process.exit(1);
}
...
const report = buildReport(results, { fixtureSet: 'corpus/backgrounds', tolerancesVersion: tolerances.version });
if (results.length === 0) {
  console.error(`verify:backgrounds: no fixtures found under ${corpus}`);
  process.exit(1);
}
