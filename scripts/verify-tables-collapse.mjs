...
const corpus = resolve('corpus/tables-collapse');
...
try {
  for (const { dir, name, raw } of fixtures()) {
    ...
    // Exclusion mask: declared maskRects/maskElements only. The collapse
    // fixtures declare none today — the border raster is compared outright.
    const mask = new Uint8Array(width * height);
...
const report = buildReport(results, { fixtureSet: 'corpus/tables-collapse', tolerancesVersion: tolerances.version });
