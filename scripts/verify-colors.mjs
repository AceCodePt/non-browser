 ...
 *
 * Renders every corpus/colors fixture with the engine, harvests the Chrome
...
const corpus = resolve('corpus/colors');
...
if (!statSync(corpus, { throwIfNoEntry: false })?.isDirectory()) {
  console.error(`verify:colors: corpus directory missing: ${corpus}`);
  process.exit(1);
}
...
const report = buildReport(results, { fixtureSet: 'corpus/colors', tolerancesVersion: tolerances.version });
if (results.length === 0) {
  console.error(`verify:colors: no fixtures found under ${corpus}`);
  process.exit(1);
}
